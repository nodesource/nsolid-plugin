...
export function generateCodexBootstrap () {
  ...
  // The recursive search is bounded to the Codex plugin cache; cwd is probed
  // only at fixed dev candidate paths. Recursively walking cwd made startup
  // exceed codex's 60s timeout when codex launched the servers from the
  // user's home directory (QA-05).
