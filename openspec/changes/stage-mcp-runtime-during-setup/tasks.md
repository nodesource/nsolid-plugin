...
- [x] Amend proposal, design, specs delta and tasks per the PR #62 review:
...
- [x] Add `semver` to `packages/core` dependencies for range evaluation;
      unparseable ranges fail closed (documented supported syntax in
      `design.md`).
- [x] `resolveNpmCommand()`: canonical Node.js-anchored candidates only — node-dir
...
- [x] `ensureMcpRemoteRuntime()`: idempotent check, staging sibling under the
      ...
      error.
- [x] Publication protocol: `O_EXCL` lock file under the runtime parent
...
- [x] Timeout handling: terminate and confirm the managed npm process tree
...
- [x] `packages/core/scripts/setup.mjs`: satisfy the runtime precondition
      (credentials-free `ensureMcpRemoteRuntime()`) before delegating to
      `install()` for opencode/pi, so neither harness bypasses
      provisioning.
- [x] `packages/core/src/cli.ts`: satisfy the same precondition before the
...
- [x] `scripts/plugin-generators.mjs`: export `MCP_REMOTE_VERSION`; embed
...
- [x] `openspec validate stage-mcp-runtime-during-setup --strict` passes after
      all final spec and task edits.
- [x] `pnpm --filter ./packages/core lint`, `pnpm --filter ./packages/core test`,
