...
- **Testing**: Use fixture paths/environments for npm, pnpm, `npx`, Volta, Yarn, Bun, workspace, broken symlink, manager-reported root mismatch, and ambiguous launchers. Verify unsupported sources produce exact-version guidance without mutation. References: Update Flow “Unsupported CLI installation source.”
...
- **Testing**: Cover all acceptance tests from the proposal on Linux-compatible fixtures and keep paths/commands portable for macOS and Windows CI. Assert credentials, non-NodeSource configurations, unrelated Codex configuration/cache entries, unrelated Antigravity manifest imports, and source identities/scopes are preserved byte-for-byte where applicable.
