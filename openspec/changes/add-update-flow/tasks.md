...
- [ ] **Description**: Implement the injected shell-free command runner, bounded/sanitized output, executable lookup, npm registry client, exact-version npm-exec/pnpm-dlx package executors, exact carried Claude/Codex marketplace version sources, and the fixed canonical GitHub-root Antigravity bundle source with explicit timeouts and validation. Never substitute the NodeSource marketplace for alternate, missing, stale, or unsupported marketplace evidence.
- **Depends on**: Task 1
